...
String string_rs (double d_rs) {
	int rs_hour = int(d_rs);
	int rs_minute = int((d_rs - rs_hour)*60);
	int rs_second = int(((d_rs - rs_hour)*60 - rs_minute)*60);
	String string_rs = String(rs_hour);
	string_rs += ":";
	if (rs_minute < 10)  string_rs += "0";
	string_rs += String (rs_minute);
	string_rs += ":";
	if (rs_second < 10)  string_rs += "0";
	string_rs += String (rs_second);
	return string_rs;
}
...
void sunRiseSet () {
	...
	daylen = day_length(DateTime.year,DateTime.month,DateTime.day,config.longitude.toFloat(),config.latitude.toFloat());
	civlen = day_civil_twilight_length(DateTime.year,DateTime.month,DateTime.day,config.longitude.toFloat(),config.latitude.toFloat());
	rs = sun_rise_set(DateTime.year,DateTime.month,DateTime.day,config.longitude.toFloat(), config.latitude.toFloat(), &rise, &set );
	rise = rise + (config.timeZone/10);
	set = set + (config.timeZone/10);
	if (config.isDayLightSaving && summerTime(absoluteActualTime)) {rise += 1; set += 1;}
